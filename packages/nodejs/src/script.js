...
// Don't auto-initialize on window.onload
window.oneInch = function(options) {
  ...
  try {
    ...
    const safeOptions = {
      ...
      defaultAmount: String(options.defaultAmount || '1.0'),
      onSwap: Boolean(options.onSwap),
      onError: Boolean(options.onError)
    };
    
    console.log('Prepared safe options:', safeOptions);

    // First stringify, then Base64 encode
    const jsonStr = JSON.stringify(safeOptions);
    console.log('JSON string:', jsonStr);
    
    const base64Encoded = btoa(jsonStr);
    console.log('Base64 encoded:', base64Encoded);

    // Create iframe with encoded options
    const iframe = document.createElement('iframe');
    iframe.src = `http://localhost:3502?options=${encodeURIComponent(base64Encoded)}`;
...
class Swap {
  constructor() {
    console.log('Swap constructor called');
    ...
    try {
      ...
      if (!querystring.options) {
        console.warn('No options found in URL');
        this.props = {};
        return;
      }

      const encodedOptions = decodeURIComponent(querystring.options);
      console.log('URL decoded options:', encodedOptions);

      if (!isValidBase64(encodedOptions)) {
        console.error('Invalid Base64 string:', encodedOptions);
        this.props = {};
        return;
      }

      const jsonStr = atob(encodedOptions);
      console.log('Base64 decoded JSON:', jsonStr);

      this.props = JSON.parse(jsonStr);
      console.log('Final parsed props:', this.props);
